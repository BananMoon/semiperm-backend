...
public abstract class SocialLoadStrategy {
     ...
     * @param accessToken
     */
    public CustomOAuth2User getOAuth2User (String accessToken) {
        HttpEntity<MultiValueMap<String, String>> request = prepareRequest(accessToken);
        CustomOAuth2User socialUserInfoResponse = null;
        try {
    ...
    protected abstract CustomOAuth2User sendRequestToSocialApi(HttpEntity<MultiValueMap<String, String>> request);
    protected abstract CustomOAuth2User makeOAuth2User(Map<String, Object> attributes);
    public abstract Account makeAccount(CustomOAuth2User oAuth2User);
}
