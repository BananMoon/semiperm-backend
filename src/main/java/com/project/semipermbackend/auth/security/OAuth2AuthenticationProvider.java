...
import com.project.semipermbackend.auth.service.SocialLoadStrategy;
...
public class OAuth2AuthenticationProvider implements AuthenticationProvider {
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        // 사용자 조회 API 호출
        SocialTypeAccessToken customAuthentication = (SocialTypeAccessToken) authentication;
        CustomOAuth2User customOAuth2User = oAuth2UserService.getCustomOAuth2User(customAuthentication);    // customAuthentication1

        // TODO 사용자 조회 결과가 없을 때 ErrorHandler로 가나??

        return SocialTypeAccessToken.builder()
                .principal(customOAuth2User)
                .authorities(customOAuth2User.getAuthorities())   // null..
                .build();
    ...
    public boolean supports(Class<?> authentication) {
        return SocialTypeAccessToken.class.isAssignableFrom(authentication);
    }
}
